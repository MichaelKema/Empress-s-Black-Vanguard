...
func _ready():
    # connect button if you didn't do it in the editor
...
func _on_build_btn_pressed() -> void:
    if _building:
        ...
        return
    if tower_scene == null:
        push_error("UI: assign tower_scene on the UI node"); return

    _building = true
    _ghost = tower_scene.instantiate()
    if _ghost.has_method("set_preview"):
        _ghost.set_preview(true)

    var parent := _get_parent()
    if parent == null:
        push_error("UI: placement parent missing"); _building = false; return

    parent.add_child(_ghost)
    _move_ghost()
    build_btn.modulate = Color(1,1,1,0.7)
...
func _unhandled_input(event):
    ...
    if event is InputEventMouseButton and event.pressed:
        if event.button_index == MOUSE_BUTTON_LEFT:
            # don’t place if cursor is over any UI Control
...
func _mouse_world() -> Vector2:
    ...
    return cam.screen_to_world(get_viewport().get_mouse_position()) if cam else get_global_mouse_position()
